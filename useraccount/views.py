...
@never_cache
def logout(request):
    request.session.pop('userlogin', None)   # remove custom key safely
...
from warranty.models import Warranty
from useraccount.models import Accounts  # Adjust import according to your app structure
...
def my_warranties(request):
    ...
    warranties = Warranty.objects.filter(user=current_user).order_by('-start_date')
    context = {
        'warranties': warranties,
    }
    return render(request, 'user/validate_warranty.html', context)
